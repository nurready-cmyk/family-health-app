...
// Ported from js/nutrition-db.js (PWA) — same data and helpers.
var NutritionDB = [
...
/**
 * Fuzzy search products by name — used to resolve free-typed food names from Telegram.
 * Exact/substring match first; falls back to matching by individual words.
 */
function searchFood(query) {
  var q = query.toLowerCase().trim();
  if (!q) return [];
  var direct = NutritionDB.filter(function (f) { return f.name.toLowerCase().indexOf(q) !== -1; });
  if (direct.length) return direct;
  var words = q.split(/\s+/).filter(Boolean);
  return NutritionDB.filter(function (f) {
...
function calculateDayNutrients(entries) {
  ...
  entries.forEach(function (entry) {
    var food = NutritionDB.filter(function (f) { return f.name === entry.foodName; })[0];
    if (!food) return;
    var ratio = (entry.grams || 100) / 100;
    ...
    Object.keys(food.vitamins).forEach(function (k) {
      vitamins[k] = (vitamins[k] || 0) + food.vitamins[k] * ratio;
    });
  });

  return { totals: totals, vitamins: vitamins };
}
